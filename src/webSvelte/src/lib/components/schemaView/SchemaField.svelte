...
<div class="border-solid border-b flex flex-row w-full py-2 border-gray-200 hover:bg-gray-100">
	<div class="w-6">
		<button
			on:click={() => {
				if (isVisible) {
	...
	<div class="grow text-gray-900 whitespace-nowrap truncate" class:text-purple-800={isAnnotation}>
		<!-- {path.at(-1)} -->
		{path.join('.')}
		{#if isAnnotation}
