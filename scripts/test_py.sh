...
# 1) `lilacai/data/dataset_test.py` to run a single file.
# 2) `lilacai/data/dataset_test.py::SelectRowsSuite` to run a test suite.
# 3) `lilacai/data/dataset_test.py::SelectRowsSuite::test_columns` to run a single test.
if [ "$1" ]; then
  TEST_PATH="$1"
else
  TEST_PATH="lilacai/"
