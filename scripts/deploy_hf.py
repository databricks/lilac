...
from lilac.config import CONFIG_FILENAME, DemoConfig
from lilac.env import data_path, env
from lilac.utils import get_dataset_output_dir, get_lilac_cache_dir, to_yaml
...
def deploy_hf(hf_username: Optional[str], hf_space: Optional[str], datasets: list[str],
  ...
  for d in datasets:
    ...
    readme = ('This dataset is generated by [Lilac](http://lilacml.com) for a HuggingFace Space: '
              f'[huggingface.co/spaces/{hf_space_org}/{hf_space_name}]'
              f'(https://huggingface.co/spaces/{hf_space_org}/{hf_space_name}).\n\n'
              'The following config was used to generate the dataset:\n'
              f'```{dataset_config_yaml}```'.encode())
    hf_api.upload_file(
    ...
    lilac_hf_datasets.append(repo_id)
